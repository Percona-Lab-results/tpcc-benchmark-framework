...
<p>
  ...
  <p>
    <strong>Buffer pool instances.</strong>
    <code>innodb_buffer_pool_instances</code> is automatically scaled:
    <code>instances\u2009=\u2009buffer_pool_GB\u2009/\u20095</code> (minimum\u20091),
    giving 16 instances at 80G. This applies to MySQL only \u2014 MariaDB uses a single
    buffer pool instance.
  </p>
  <p>
    <strong>I/O and storage.</strong>
    <code>innodb_io_capacity</code> is set to 10,000 to fully utilise NVMe storage and
    avoid I/O throttling during background flushing. Direct I/O bypasses the operating
    system page cache, allowing each engine to manage its own memory through the buffer
    pool without double-caching.
  </p>
  <p>
    <strong>Redo log.</strong>
    The InnoDB redo log is set to 32 GiB \u2014 deliberately oversized so that log
    capacity is never a bottleneck and no engine is limited by checkpoint pressure.
  </p>
  <p>
    <strong>Durability.</strong>
    Binary logging is configured for full safety:
    <code>sync_binlog\u2009=\u20091</code> fsyncs every transaction to the binlog before
    commit, and <code>innodb_flush_log_at_trx_commit\u2009=\u20091</code> flushes the redo
    log on each commit \u2014 the most durable setting at the cost of some throughput.
  </p>
  <p>
