...
def infer_db(label: str, version: str) -> str:
    ...
    if "seekdb" in label_lower or "seekdb" in ver_lower:
        return "SeekDB"
...
def parse_nopm_samples(content: str, start_ts: str) -> list[dict]:
    """Parse nopm_samples.csv (HammerDB native NOPM output) into qps-like rows.
    The tpm column contains per-minute TPM; convert to per-second TPS = tpm/60.
    Generate synthetic timestamps starting from start_ts, 1 second apart."""
    from datetime import datetime, timedelta
    rows = []
    t0 = datetime.fromisoformat(start_ts) if start_ts else datetime(2026, 1, 1)
    sec = 0
    for line in content.splitlines():
        if "NOPM_SAMPLE" not in line:
            continue
        ...
        try:
            ...
            continue
        tps = tpm / 60.0
        ts = (t0 + timedelta(seconds=sec)).strftime("%Y-%m-%d %H:%M:%S")
        rows.append({"timestamp": ts, "tps": str(round(tps, 2))})
        sec += 1
    return rows


def summarize_nopm_tps(rows: list[dict], rampup_seconds: int) -> dict:
    """Compute TPS stats from nopm_samples-derived rows."""
    tps_values = [float(r["tps"]) for r in rows[rampup_seconds:] if float(r["tps"]) > 0]
    if not tps_values:
        return {}
    return {
        "avg": round(sum(tps_values) / len(tps_values), 1),
...
def main():
    ...
    for run_dir in run_dirs:
        ...
        rampup = params.get("rampup_seconds", 60)
        tps_stats = summarize_tps(qps_rows, rampup)

        # Fallback: if qps.csv has zero TPS, try nopm_samples.csv
        if tps_stats.get("avg", 0) == 0:
            nopm_raw = git_show(f"{run_dir}/nopm_samples.csv")
            if nopm_raw:
                nopm_rows = parse_nopm_samples(nopm_raw, params.get("timestamp"))
                if nopm_rows:
                    tps_stats = summarize_nopm_tps(nopm_rows, rampup)
                    qps_rows = nopm_rows
