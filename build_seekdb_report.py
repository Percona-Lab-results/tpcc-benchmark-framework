"""
Build static HTML report comparing MySQL 9.7 vs SeekDB
from BP sweep benchmark runs.
"""
import os as _os
...
def engine_id(run: dict) -> str | None:
    """Classify a run into mysql97 or seekdb."""
    ver = (run.get("version") or "").lower()
    db  = run.get("db", "").lower()
    label = run.get("label", "").lower()
    if "seekdb" in label or "seekdb" in ver or db == "seekdb":
        return "seekdb"
    if "9.7" in ver or "mysql97" in label:
        return "mysql97"
    return None
...
# ══════════════════════════════════════════════════════════════════════════════
#  DATASET 3 — TPS timeseries for representative runs (BP 80G sweep)
# ══════════════════════════════════════════════════════════════════════════════
def best_bp_run(eid):
    cands = [
        r for r in bp_runs
...
ts_runs = {eid: best_bp_run(eid) for eid in ENGINE_IDS}
...
<head>
<meta charset="UTF-8">
<title>MySQL 9.7 vs SeekDB \u2014 TPROC-C Benchmark Report</title>
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{
    font-family: 'Inter', 'Segoe UI', system-ui, -apple-system, sans-serif;
    background: #080b14; color: #cdd5e0; line-height: 1.65;
  }}
  a {{ color: #5b9bd5; }}
  .page {{ max-width: 1120px; margin: 0 auto; padding: 48px 28px 96px; }}
  header {{ border-bottom: 1px solid #1c2438; padding-bottom: 24px; margin-bottom: 52px; }}
  header h1 {{ font-size: 1.75rem; font-weight: 700; color: #eef0f8; letter-spacing: -0.025em; }}
  header .subtitle {{ color: #4a5870; font-size: 0.875rem; margin-top: 8px; }}
  .pills {{ display: flex; gap: 8px; flex-wrap: wrap; margin-top: 18px; }}
  .pill {{ font-size: 0.74rem; padding: 3px 11px; border-radius: 20px; border: 1px solid #1c2438; color: #4a5870; }}
  section {{ margin-bottom: 64px; }}
  section h2 {{ font-size: 0.72rem; font-weight: 700; color: #3d5a78; text-transform: uppercase; letter-spacing: 0.13em; margin-bottom: 22px; padding-bottom: 10px; border-bottom: 1px solid #1c2438; }}
  section h3 {{ font-size: 0.92rem; font-weight: 600; color: #9aa8be; margin: 34px 0 12px; }}
  ...
  .chart-caption {{ font-size: 0.74rem; color: #364558; margin-top: 7px; margin-bottom: 28px; font-style: italic; }}
  .data-table {{ width: 100%; border-collapse: collapse; font-size: 0.84rem; margin-top: 16px; }}
  .data-table th {{ text-align: left; color: #3d5a78; padding: 10px 14px; border-bottom: 1px solid #1c2438; font-weight: 600; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.07em; }}
  .data-table td {{ padding: 8px 14px; border-bottom: 1px solid #0d1117; }}
  ...
  .callout {{ background: #0d1117; border-left: 3px solid #1c3a58; border-radius: 0 10px 10px 0; padding: 14px 20px; font-size: 0.875rem; color: #7a8898; margin: 24px 0; max-width: 820px; }}
  .callout strong {{ color: #cdd5e0; }}
  .cfg-table {{ font-size: 0.78rem; }}
  .cfg-table .cfg-section td {{ background: #060810; color: #3d5a78; font-size: 0.68rem; text-transform: uppercase; letter-spacing: 0.11em; padding: 12px 14px 5px; font-weight: 700; border-bottom: none; }}
  ...
  .cfg-table td:not(:first-child) {{ font-family: 'Consolas','SF Mono',monospace; color: #667080; font-size: 0.76rem; }}
  .cfg-na {{ color: #2a3348; font-style: italic; }}
  footer {{ border-top: 1px solid #1c2438; padding-top: 24px; font-size: 0.74rem; color: #283040; text-align: center; }}
</style>
</head>
<body>
<div class="page">
...
<header>
  <h1>MySQL 9.7 vs SeekDB \u2014 TPROC-C Benchmark Report</h1>
...
<section>
  ...
  <p>
    Both engines ran TPROC-C with 64 virtual users and buffer pool varied from 10 GiB to 80 GiB.
    The dataset is 1000 warehouses (~100 GB working set), so an 80 GiB pool covers ~80% of hot data.
  </p>
  <div class="chart"><img src="data:image/png;base64,{img_bp_line}" alt="BP sweep line chart"></div>
  <div class="chart-caption">Figure 1 \u2014 Average NOTPM vs buffer pool size.</div>
  ...
  <table class="data-table">
    <thead><tr><th>BP Size</th>{engine_th()}<th>\u0394</th></tr></thead>
...
<section>
  ...
  <p>
    Per-second NOTPM for the best BP 80G run from each engine.
    Thin lines are raw 1-second samples; thick lines are 60-second rolling averages.
  </p>
  <div class="chart"><img src="data:image/png;base64,{img_ts}" alt="NOTPM timeseries"></div>
  <div class="chart-caption">Figure 3 \u2014 NOTPM over elapsed time. Ramp-up excluded.</div>
</section>
...
<section>
  <h2>Methodology</h2>
  <p><strong>Benchmark:</strong> TPROC-C via HammerDB 4.12.</p>
  ...
  on the same host. Container settings: <code>MEMORY_LIMIT</code> varied per sweep step (10G\u201380G),
  <code>LOG_DISK_SIZE=32G</code>, <code>CPU_COUNT=0</code> (all CPUs),
  <code>DATAFILE_MAXSIZE=512G</code>. Container was restarted for each step.
  Connected via localhost:2881. SeekDB is based on OceanBase and presents a MySQL-compatible
  wire protocol, allowing HammerDB to connect using the standard MySQL driver.</p>
  ...
  <code>innodb_buffer_pool_size</code> parameter was varied directly. For SeekDB, the Docker
  <code>MEMORY_LIMIT</code> parameter was varied per step (10G\u201380G), controlling OceanBase\u2019s
  total memory allocation. The container was restarted for each step.</p>
